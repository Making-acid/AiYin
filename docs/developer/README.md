...
Beta 1.2 的内部语义版本为 `1.2.0-beta.1`。版本目前由测试约束在后端、前端、桌面项目、安装器和发布脚本之间保持一致。

```powershell
.\desktop\build-webview-release.ps1
```

发布脚本会在仓库外创建 `IELTS-Speaking-WebView2-Beta-1.2-Release`，其中：
