...
This notice describes the current Beta 1.0 desktop/local-web build. A modified or
hosted deployment may behave differently. / 本说明适用于当前 Beta 1.0 桌面版及本地网页版本；
...
| Text-to-speech | Browser mode uses a voice supplied by the browser or operating system; network use depends on that implementation. If you explicitly enable Azure Speech, character reply text is sent to Microsoft for synthesis. The Azure key remains in the local backend configuration and the page receives only a short-lived authorization token. Microsoft's terms, billing, retention, and privacy rules apply. |
